...
CREATE TABLE users (
    ...
    created_at timestamp without time zone NOT NULL,
    updated_at timestamp without time zone NOT NULL,
    CONSTRAINT email_must_be_valid_format CHECK (((email)::text ~* '^(?=[A-Za-z0-9][A-Za-z0-9@._%+-]{5,253}$)[A-Za-z0-9._%+-]{1,64}@(?:(?=[A-Za-z0-9-]{1,63}.)[A-Za-z0-9]+(?:-[A-Za-z0-9]+)*.){1,8}[A-Za-z]{2,63}$'::text))
);
